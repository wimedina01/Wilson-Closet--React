...
import {
  ...
  pushAllItems, listSpreadsheets, createSpreadsheet, checkForChanges,
} from './lib/sheets.js'
...
// ── Check if URL is a shared gallery
function getGalleryId() {
  const h = location.hash
  return h.startsWith('#gallery/') ? h.replace('#gallery/', '') : null
}
  ...
  if (page === 'gallery' && galleryId) {
    return (
      <>
        ...
        <GalleryPage
          groupId={galleryId} groups={groups} items={items}
    ...
    <>
      ...
      <div className="shell">
        ...
        <main className="main">
            ...
            <div className="page">
              <div className="ph">
                ...
                <div className="ph-actions">
                  ...
                  >🔄 Sync</button>
                  {lastSync && <span style={{ fontSize: 10, color: 'var(--ink3)' }}>Synced {lastSync}</span>}
                  <button className="btn btn-secondary btn-sm" onClick={() => goPage('share')}>🔗 Share</button>
                  <button className="btn btn-primary btn-sm" onClick={() => { setEditItem(null); setShowAddModal(true) }}>＋ Add Item</button>
                </div>
              </div>
          ...
          {/* ── SHARE PAGE */}
          {page === 'share' && (
            <SharePage groups={groups} items={items} toast={toast} />
          )}

          {/* ── NOTIFICATIONS PAGE */}
          {page === 'notifications' && (
            <NotificationsPage notifications={notifications} onUpdate={setNotifications} />
          )}
