...
export default function Sidebar({ groups, items, gToken, gUser, activePage, activeGroup, onPage, onGroup, onCat, onGoogleAuth, onAddGroup, isOpen }) {
  const unread = 0 // passed from parent in full app
  ...
  return (
    <nav className={`sidebar ${isOpen ? 'open' : ''}`}>
      ...
      <div className="sb-logo">
        ...
        <div className="sb-logo-sub">Wardrobe Inventory</div>
      </div>

      {/* User */}
      <div className="sb-user" onClick={onGoogleAuth}>
        <div className="sb-avatar" id="sb-av">
          {gUser?.picture
      ...
      <div className="sb-scroll">
        ...
        <button className="sb-add-group" onClick={onAddGroup}>
          <span style={{ fontSize: 15 }}>＋</span> Add Group
        </button>
        ...
        <button className="nav-btn" onClick={() => onCat('all')}>
          <span className="ni">✦</span>All Items
        </button>
        {CATEGORIES.map(c => (
          <button key={c} className="nav-btn" onClick={() => onCat(c)}>
            <span className="ni">{CAT_EMOJI[c]}</span>{c}
          </button>
        ))}
      </div>

      {/* Footer — Google connect */}
      <div className="sb-footer">
        <div className={`g-btn ${gToken ? 'connected' : ''}`} onClick={onGoogleAuth}>
          ...
          <span>{gToken ? '✓ ' + (gUser?.email || 'Connected') : 'Connect Google'}</span>
        </div>
      </div>
    </nav>
  )
