...
// Format: #gallery/{groupId}/{sheetId}
function getGalleryId() {
  const h = location.hash
  if (!h.startsWith('#gallery/')) return null
  const parts = h.replace('#gallery/', '').split('/')
  return parts[0] || null
}
function getGallerySheetId() {
  const h = location.hash
  if (!h.startsWith('#gallery/')) return null
  const parts = h.replace('#gallery/', '').split('/')
  return parts[1] || null
    ...
    <>
      ...
      <div className="shell">
        ...
        <main className="main">
          ...
          {/* ── SHARE PAGE */}
          {page === 'share' && (
            <SharePage groups={groups} items={items} toast={toast} sheetId={sheetId} />
          )}
