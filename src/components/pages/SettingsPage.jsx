...
import { pushSettings, pullSettings, formatSheet, deleteSpreadsheet } from '../../lib/sheets.js'
...
export default function SettingsPage({
  ...
  const [delConfName, setDelConfName]  = useState('')
  ...
  async function handleDeleteAll() {
    ...
    try {
      ...
      await deleteSpreadsheet(sheetId, gToken)
      // Delete the Wilson Closet app folder
      const appFolderId = localStorage.getItem('wc_appfolderid')
      if (appFolderId) {
        await gDelete(`https://www.googleapis.com/drive/v3/files/${appFolderId}`, gToken)
      }
      // Clear all local state
      ['wc_items','wc_groups','wc_notifs','wc_locs','wc_token','wc_user',
       'wc_sheetid','wc_appfolderid','wc_photofolderid','wc_sheetsfolderid'].forEach(k => localStorage.removeItem(k))
  ...
  return (
    <div className="page">
      ...
      <div className="cs">
        ...
        <div className="ss" style={{ borderColor: 'rgba(255,92,138,.15)', background: 'var(--danger-lt)' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            <div>
              ...
              <div style={{ fontSize: 10, color: 'var(--ink3)', marginTop: 4 }}>Removes items from this device only. Sheet is not affected.</div>
            </div>
            {gToken && sheetId && (
              <div style={{ borderTop: '1px solid rgba(255,92,138,.15)', paddingTop: 10, marginTop: 2 }}>
                <button className="btn btn-danger btn-sm" onClick={() => { setDelConfName(''); setShowDelModal(true) }}>
                  💀 Delete Sheet &amp; Drive Folder
                </button>
                <div style={{ fontSize: 10, color: 'var(--ink3)', marginTop: 4 }}>
                  Permanently deletes the Google Sheet and Wilson Closet Drive folder including all photos. This cannot be undone.
                </div>
              </div>
            )}
          </div>
        ...
        {showDelModal && (
          <div className="overlay" onClick={e => e.target === e.currentTarget && setShowDelModal(false)}>
            <div className="modal" style={{ maxHeight: '60vh' }}>
              <div className="modal-handle" />
              <div className="modal-header">
                <div className="modal-title" style={{ color: 'var(--danger)' }}>💀 Permanent Deletion</div>
                <button className="ib" onClick={() => setShowDelModal(false)}>✕</button>
              </div>
              <div className="modal-body">
                <div style={{ background: 'var(--danger-lt)', border: '1px solid rgba(255,92,138,.2)', borderRadius: 'var(--r)', padding: '12px 14px', marginBottom: 16 }}>
                  <div style={{ fontSize: 12, color: 'var(--danger)', fontWeight: 700, marginBottom: 6 }}>⚠ This will permanently delete:</div>
                  <div style={{ fontSize: 12, color: 'var(--ink2)', lineHeight: 1.8, fontFamily: 'JetBrains Mono,monospace' }}>
                    • Your Google Sheet and all item data<br/>
                    • Wilson Closet Drive folder and all photos<br/>
                    • All local app data on this device
                  </div>
                </div>
                <div className="fg">
                  <label className="fl">Type your name to confirm</label>
                  <input
                    className="fi"
                    value={delConfName}
                    onChange={e => setDelConfName(e.target.value)}
                    placeholder={gUser?.name || 'Your name'}
                    autoFocus
                  />
                  {gUser?.name && (
                    <div style={{ fontSize: 10, color: 'var(--ink3)', marginTop: 4 }}>
                      Type exactly: <strong style={{ color: 'var(--ink)' }}>{gUser.name}</strong>
                    </div>
                  )}
                </div>
              </div>
              <div className="modal-footer">
                <button className="btn btn-secondary" onClick={() => setShowDelModal(false)}>Cancel</button>
                <button
                  className="btn btn-danger"
                  style={{ flex: 1, justifyContent: 'center' }}
                  disabled={delConfName.trim().toLowerCase() !== (gUser?.name || '').toLowerCase() || deleting}
