 ...
 */
export default function GalleryPage({ groupId, sheetId, groups, items: ownerItems, token, gToken, gUser }) {
  ...
  async function submitRequest() {
    ...
    // Save notification to localStorage (owner will see it on their device)
    ...
    // Send single email if owner is signed in on this browser
    if (gToken && gUser?.email) {
      ...
      const body = [
        `${reqName} (${reqEmail}) has requested:`, '', itemLines,
        '', reqMsg ? `Message: "${reqMsg}"` : '', '', '— Wilson Closet',
      ].join('\n')
